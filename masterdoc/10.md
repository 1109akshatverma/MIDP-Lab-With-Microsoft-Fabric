...
1. Click on the **Workspaces (1)** and select **My workspace (2)**.

   ![](../media/10/00.png)

2. Click on the **+ New item (1)** button and select **Data pipeline (2)**.

   ![](../media/221124(6).png)

3. In the **New pipeline** dialog box, specify the name as **IngestDataFromSourceToLakehouse (1)** and select **Create (2)**. A new data factory pipeline is created and opened.

   ![new-data-pipeline](../media/10/02.png)

4. On your newly created data factory pipeline, select **Copy data assistant** to add an activity to the pipeline.

   ![add-pipeline-copy-data](../media/10/03at.png)

5. Next, set up an HTTP connection to import the sample World Wide Importers data into the Lakehouse. From the list of Choose data sources, search for **Http (1)** and select **Http (2)**.

   ![add-pipeline-copy-data](../media/10/03ab.png)

6. In the **Connect to data source** window, enter the details from the table below and select **Next (6)**.
...
8. In **Choose data destination**, select your lakehouse as **wwilakehouse**.

   ![add-pipeline-copy-data](../media/10/03ae.png)

9. In the **Connect to data destination** window, click on the **Browse** then **OK** and click **Next**. This will write the data to the Files section of the lakehouse.

   ![add-pipeline-copy-data](../media/10/03af_new.png)

10. Click **Next**. In the **Review + Save** tab, the copy summary should be set as **Binary to Binary**. Click **Save + Run**. You can schedule pipelines to refresh data periodically. In this tutorial, we only run the pipeline once. The data copy process takes approximately 10-15 minutes to complete.

    ![add-pipeline-copy-data](../media/10/03ah.png)
11. You can monitor the pipeline execution and activity in the **Output** tab. You can also view detailed data transfer information by selecting the glasses icon next to the pipeline name, which appears when you hover over the name

   ![add-pipeline-copy-data](../media/10/03ai.png)

    >**Note**: The process will take up to 20 minutes to complete. 

12. Now, navigate back to **Fabric workspace** and select **wwilakehouse (1)** to launch **Lakehouse explorer**.

    ![add-pipeline-copy-data](../media/09/22a.png)

13. Validate that in the Lakehouse Explorer view, a new folder **WideWorldImportersDW** has been created and data for all the tables have been copied there.

    >**Note**: Refresh the Files section from the drop-down. 

    ![add-pipeline-copy-data](../media/10/03ak.png)

14. The data is generated within the **Files** section of the Lakehouse Explorer. It appears inside a new folder named with a GUID. To make it easier to identify, rename this folder to **wwi-raw-data** by clicking on the **Ellipsis (...) (1)** next to the folder and choosing **Rename (2)** from the menu.
